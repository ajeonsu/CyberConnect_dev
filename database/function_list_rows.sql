create table public.function_list_rows (
  ...
  status public.function_status not null default 'Need to be checked'::function_status,
  completion_dev public.check_status not null default ''::check_status,
  completion_client public.check_status not null default ''::check_status,
...
where (function_code <> ''::text);

create trigger trg_function_list_updated_at BEFORE
